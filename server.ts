...
app.post("/api/admin/bypass", async (req, res) => { if (!supabaseAdmin) return res.status(500).json({error: "no admin"}); const caller = await getCaller(req); if (!caller) return res.status(401).json({error:"no auth"}); await supabaseAdmin.from("profiles").upsert({ id: caller.id, email: caller.email, papel: "diretor_1", is_admin: true }); res.json({success:true}); });
...
app.post("/api/gemini/chat", async (req, res) => {
  const { prompt, chatHistory = [], regras = [], ucs = [], docentes = [], salas = [], geminiApiKey, geminiModel } = req.body;
...
- Regras Ativas: ${JSON.stringify(regras)}
- Unidades Curriculares (UCs): ${JSON.stringify(ucs)}
- Corpo de Docentes: ${JSON.stringify(docentes)}
- Salas e Caracter?sticas: ${JSON.stringify(salas)}
...
[REGRA_DETETADA]
{
  ...
  "config": { 
    "motor": {
      "plDiasPermitidos": ["Quarta", "Quinta", "Sexta"],
      "ucConflitos": [["SIGLA1", "SIGLA2"]],
      "maxTPporMancha": 2,
      "semanasSoTurmaA": [8, 9, 10, 11, 12, 13, 14, 15],
      "semanasSoTurmaB": [16, 17, 18, 19, 20, 21, 22, 23],
      "restricoesUC": [{"siglas": ["SIGLA1", "SIGLA2"], "diasProibidos": ["Quinta", "Sexta"], "periodosProibidos": ["tarde"], "tipos": ["PL"], "semanasRestritas": [8, 9, 10, 11, 12]}]
    }
  },
  "peso": 5,
  "ativa": true
...
[FIM_REGRA]
Inclua em 'config.motor' apenas as restrições que façam sentido para o pedido. Se o pedido afetar várias UCs (ex: "do 2º ano"), DEVE mapear e incluir as 'siglas' de TODAS as UCs correspondentes na array 'siglas', baseando-se na lista de UCs fornecida. Se não for relevante detetar uma regra nova, não coloque estes blocos especiais.
`;
...
Pode clicar em "Adicionar Regra" no assistente acima para ativ?-la no motor de otimização académica e testar instantaneamente!`;
    ...
    const finalApiKey = geminiApiKey || process.env.GEMINI_API_KEY || "dummy_key";
    ...
    let errorMsg = "Erro ao comunicar com o Assistente Gemini: " + error.message;
...
app.post("/api/gemini/test", async (req, res) => {
  ...
  if (!geminiApiKey) return res.status(400).json({ error: "Chave não fornecida." });
  ...
  try {
    ...
    await localAi.models.generateContent({
      model: geminiModel || "gemini-3.5-flash",
      contents: "Diz apenas 'ok'",
    });
    res.json({ ok: true });
  } catch (error: any) {
    res.status(401).json({ error: "Chave ou modelo inválido: " + (error.message || String(error)) });
  }
});
