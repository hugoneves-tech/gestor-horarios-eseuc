 ...
 */
export function distribuirBlocos(totalBlocos: number, semanas: SemanaInfo[]): number[] {
...
export function calcularPlano(uc: UC, semanas: SemanaInfo[]): PlanoSemanal[] {
  ...
  return semanas.map((semana, i) => {
    const manual = uc.planoDistribucao?.[semana.numero];
    return {
      semana,
      blocoT:  manual?.blocoT ?? distT[i],
      blocoTP: manual?.blocoTP ?? distTP[i],
      blocoPL: manual?.blocoPL ?? distPL[i],
      blocoS:  manual?.blocoS ?? distS[i],
    };
  });
}
