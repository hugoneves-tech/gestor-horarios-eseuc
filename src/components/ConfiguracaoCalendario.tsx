...
function DistribuicaoDetalhe({ anoSem, ucs, feriados }: DetalheProps) {
  ...
  }, [uc, anoSem.dataInicioSemestre, feriados]);
          ...
          <tbody>
            {plano.map(item => {
              ...
              return (
                <tr
                  key={semana.numero}
                  className={temFeriado ? "bg-rose-50/60" : "hover:bg-stone-50"}
                >
                  ...
                  <td className="p-2 border-b border-stone-100 text-center">
                    <span className={`font-bold ${semana.diasUteis < 5 ? "text-amber-600" : "text-stone-500"}`}>
                      {semana.diasUteis}/5
                    </span>
                  </td>
                  {tipos.includes("T")  && (
                    <td className="p-2 border-b border-stone-100 text-center font-bold text-blue-700">
                      {blocoT  > 0 ? `${blocoT  * 2}h` : <span className="text-stone-200 font-normal">—</span>}
                    </td>
                  )}
                  {tipos.includes("TP") && (
                    <td className="p-2 border-b border-stone-100 text-center font-bold text-teal-700">
                      {blocoTP > 0 ? `${blocoTP * 2}h` : <span className="text-stone-200 font-normal">—</span>}
                    </td>
                  )}
                  {tipos.includes("PL") && (
                    <td className="p-2 border-b border-stone-100 text-center font-bold text-amber-700">
                      {blocoPL > 0 ? `${blocoPL * 2}h` : <span className="text-stone-200 font-normal">—</span>}
                    </td>
                  )}
                  {tipos.includes("S") && (
                    <td className="p-2 border-b border-stone-100 text-center font-bold text-purple-700">
                      {blocoS  > 0 ? `${blocoS  * 2}h` : <span className="text-stone-200 font-normal">—</span>}
                    </td>
                  )}
