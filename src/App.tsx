...
export default function App() {
  ...
  const [testGeminiError, setTestGeminiError] = useState<string | null>(null);

  const testarEGuardarGeminiKey = async () => {
    const k = geminiKeyDraft.trim();
    if (!k) {
      guardarGeminiKey("");
      setShowGeminiKeyPanel(false);
      setTestGeminiError(null);
      showToast("Chave Gemini removida.");
      return;
    }
    ...
    try {
      const resp = await fetch("/api/gemini/test", {
        ...
        body: JSON.stringify({ geminiApiKey: k, geminiModel }),
      });
      ...
      if (resp.ok && data.ok) {
        ...
        setShowGeminiKeyPanel(false);
        setTestGeminiError(null);
        showToast("✓ Conexão bem-sucedida! Chave guardada no browser.");
      } else {
        setTestGeminiError(data.error || "Chave inválida. Verifique e tente novamente.");
      }
    } catch (err: any) {
      setTestGeminiError(err.message || "Erro de rede ao testar a chave.");
    } finally {
  ...
  const handleAddRegra = () => {
    ...
    const item: RegraHorario = {
      ...
      config: {
        ...(newRegra.config || {}),
        traducaoSimples: newRegra.config?.traducaoSimples || "Regra criada de forma simples e intuitiva na consola do coordenador."
      },
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="space-y-1">
          ...
          {(() => {
            ...
            const siglas = Array.from(new Set(restricoes.flatMap((r: any) => r.siglas || [])));
            if (siglas.length === 0) return null;
            
            return (
              <div className="mt-1.5 flex flex-wrap gap-1 items-center">
                <span className="text-[9px] font-bold text-amber-800 mr-1 uppercase">UCs Limitadas:</span>
                {siglas.map((sig: any) => (
                  <span key={sig} className="text-[9px] font-mono font-bold bg-amber-50 text-amber-800 border border-amber-200 px-1 py-0.5 rounded">
                    {sig}
                  </span>
                ))}
              </div>
          ...
          <div className="fixed inset-0 bg-stone-950/70 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in text-xs">
            <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-stone-200 flex flex-col max-h-[85vh] text-xs">
              ...
              <div className="flex-1 overflow-y-auto pr-1 space-y-4">
                ...
                {/* Limites de aulas em simultâneo */}
                <div className="bg-rose-50/20 border border-rose-100 p-3.5 rounded-xl space-y-2">
                  <div className="flex items-center gap-1.5 text-rose-800 font-serif font-bold text-xs">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="block text-[10px] font-bold text-stone-500 mb-1">Máx T em Simultâneo</label>
                      <input
                        type="number"
                        min={0}
                        placeholder="Sem limite"
                        value={activeEditingUc.maxSimultaneoT ?? ""}
                        onChange={(e) => {
                          const val = e.target.value === "" ? undefined : Number(e.target.value);
                          const updated = ucs.map(u => u.id === editingUcId ? { ...u, maxSimultaneoT: val } : u);
                          setUcs(updated);
                        }}
                        className="w-full bg-white border border-stone-200 rounded-lg px-2.5 py-1.5 font-mono font-medium"
                      />
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-[#148A96] mb-1">Máx TP em Simultâneo</label>
                      <input
                        type="number"
                        min={0}
                        placeholder="Sem limite"
                        value={activeEditingUc.maxSimultaneoTP ?? ""}
                        onChange={(e) => {
                          const val = e.target.value === "" ? undefined : Number(e.target.value);
                          const updated = ucs.map(u => u.id === editingUcId ? { ...u, maxSimultaneoTP: val } : u);
                          setUcs(updated);
                        }}
                        className="w-full bg-white border border-stone-200 rounded-lg px-2.5 py-1.5 font-mono font-medium"
                      />
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-teal-800 mb-1">Máx PL em Simultâneo</label>
                      <input
                        type="number"
                        min={0}
                        placeholder="Sem limite"
                        value={activeEditingUc.maxSimultaneoPL ?? ""}
                        onChange={(e) => {
                          const val = e.target.value === "" ? undefined : Number(e.target.value);
                          const updated = ucs.map(u => u.id === editingUcId ? { ...u, maxSimultaneoPL: val } : u);
                          setUcs(updated);
                        }}
                        className="w-full bg-white border border-stone-200 rounded-lg px-2.5 py-1.5 font-mono font-medium"
                      />
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                ...
                <div className="bg-stone-50 p-3 rounded-xl border border-stone-200 space-y-1.5 max-h-40 overflow-y-auto">
                  {ucs
                    .filter(u => {
                      const ruleAnos = anosDaRegra(regraEmEdicao!);
                      const ruleCursos = cursosDaRegra(regraEmEdicao!);
                      const matchesAno = ruleAnos.length === 0 || ruleAnos.includes(Number(u.anoCurricular));
                      const matchesCurso = ruleCursos.length === 0 || ruleCursos.includes(u.cursoId);
                      return matchesAno && matchesCurso;
                    })
                    .map(u => {
                      const motor = (regraEmEdicao!.config as any)?.motor || {};
                      const restricoes = Array.isArray(motor.restricoesUC) ? motor.restricoesUC : [];
                      const isSelected = restricoes.some((r: any) => r.siglas?.includes(u.sigla));
                      
                      const handleToggleUc = () => {
                        const currentMotor = { ...(regraEmEdicao!.config?.motor || {}) };
                        let currentRestricoes = Array.isArray(currentMotor.restricoesUC) ? [...currentMotor.restricoesUC] : [];
                        
                        if (isSelected) {
                          currentRestricoes = currentRestricoes.map((r: any) => {
                            if (!r.siglas) return r;
                            const nextSiglas = r.siglas.filter((s: string) => s !== u.sigla);
                            return { ...r, siglas: nextSiglas };
                          }).filter((r: any) => r.siglas && r.siglas.length > 0);
                        } else {
                          if (currentRestricoes.length === 0) {
                            currentRestricoes.push({
                              siglas: [u.sigla],
                              diasProibidos: [],
                              periodosProibidos: [],
                              tipos: []
                            ...
                            currentRestricoes[0] = {
                              ...currentRestricoes[0],
                              siglas: [...(currentRestricoes[0].siglas || []), u.sigla]
                            };
                          }
                        }
                        ...
                        setRegraEmEdicao({
                          ...
                          config: {
                            ...
                            motor: {
                              ...currentMotor,
                              restricoesUC: currentRestricoes
                            }
                          }
                        });
                      };
                      ...
                      return (
                        <label key={u.id} className="flex items-center gap-2 cursor-pointer p-1 hover:bg-stone-100 rounded">
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={handleToggleUc}
                            className="rounded text-stone-900 focus:ring-stone-600"
                          />
                          <span className="font-mono text-[10px] font-bold text-stone-700">[{u.sigla}]</span>
                          <span className="text-stone-600 font-light text-[11px] truncate">{u.nome} ({u.anoCurricular}º ano)</span>
                        </label>
                      );
                    })}
                  {ucs.filter(u => {
                    ...
                    const matchesCurso = ruleCursos.length === 0 || ruleCursos.includes(u.cursoId);
                    return matchesAno && matchesCurso;
                  }).length === 0 && <span className="text-stone-400 italic text-[11px]">Nenhuma UC disponível para o âmbito selecionado.</span>}
                </div>
                <p className="text-[9px] text-stone-400 mt-1">Ao selecionar UCs específicas, pode definir dias ou períodos proibidos abaixo para essas disciplinas.</p>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div className="bg-amber-50/50 p-3 rounded-xl border border-amber-100 space-y-2">
                    <span className="block text-[10px] font-bold text-amber-800 uppercase tracking-wider">Definições da Restrição para as UCs Selecionadas</span>
                    
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] font-bold text-stone-500 mb-1">Dias Proibidos</label>
                        <div className="flex flex-wrap gap-1">
                          {["Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado"].map(dia => {
                            const isProibido = restricoes.some((r: any) => r.diasProibidos?.includes(dia));
                            const handleToggleDia = () => {
                              const nextRestricoes = restricoes.map((r: any) => {
                                const currentDias = r.diasProibidos || [];
                                const nextDias = currentDias.includes(dia)
                                  ? currentDias.filter((d: string) => d !== dia)
                                  : [...currentDias, dia];
                                return { ...r, diasProibidos: nextDias };
                              });
                              setRegraEmEdicao({
                                ...regraEmEdicao!,
                                config: {
                                  ...(regraEmEdicao!.config || {}),
                                  motor: { ...motor, restricoesUC: nextRestricoes }
                                }
                              });
                            };
                            return (
                              <button
                                ...
                                className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${isProibido ? "bg-amber-100 text-amber-800 border-amber-200" : "bg-white text-stone-500 border-stone-200 hover:bg-stone-50"}`}
                              >
                                {dia.slice(0, 3)}
                              </button>
                            );
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-stone-500 mb-1">Períodos Proibidos</label>
                        <div className="flex gap-2">
                          {["manha", "tarde"].map(p => {
                            const isProibido = restricoes.some((r: any) => r.periodosProibidos?.includes(p));
                            const handleTogglePeriodo = () => {
                              const nextRestricoes = restricoes.map((r: any) => {
                                const currentPers = r.periodosProibidos || [];
                                const nextPers = currentPers.includes(p)
                                  ? currentPers.filter((x: string) => x !== p)
                                  : [...currentPers, p];
                                return { ...r, periodosProibidos: nextPers };
                              });
                              setRegraEmEdicao({
                                ...regraEmEdicao!,
                                config: {
                            ...
                            return (
                              <button
                                ...
                                className={`flex-1 py-1 rounded text-[10px] font-semibold border capitalize ${isProibido ? "bg-amber-100 text-amber-800 border-amber-200" : "bg-white text-stone-500 border-stone-200 hover:bg-stone-50"}`}
                              >
                                {p}
                              </button>
                            );
                    ...
                    <div className="grid grid-cols-1 gap-3 pt-1">
                      <div>
                        <label className="block text-[10px] font-bold text-stone-500 mb-1">Aplicar apenas a Tipo de Aula</label>
                        <div className="flex flex-wrap gap-1">
                          {["T", "TP", "PL", "S"].map(tipo => {
                            const isIncluded = restricoes.some((r: any) => r.tipos?.includes(tipo));
                            const handleToggleTipo = () => {
                              const nextRestricoes = restricoes.map((r: any) => {
                                const currentTipos = r.tipos || [];
                                const nextTipos = currentTipos.includes(tipo)
                                  ? currentTipos.filter((t: string) => t !== tipo)
                                  : [...currentTipos, tipo];
                                return { ...r, tipos: nextTipos };
                              });
                              setRegraEmEdicao({
                                ...regraEmEdicao!,
                                config: {
                            ...
                            return (
                              <button
                                ...
                                className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold border ${isIncluded ? "bg-amber-100 text-amber-800 border-amber-200" : "bg-white text-stone-500 border-stone-200 hover:bg-stone-50"}`}
                              >
                                {tipo}
                              </button>
                            );
              ...
              {isAddingRegra && (
                <div className="bg-stone-50 p-5 rounded-2xl border border-stone-150 grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in text-xs">
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-4xs font-bold uppercase tracking-wider text-stone-400 mb-1">Âmbito</label>
                        <select
                    ...
                    <div>
                      <label className="block text-4xs font-bold uppercase tracking-wider text-stone-400 mb-1">Unidades Curriculares (UCs) específicas</label>
                      <div className="bg-white p-2.5 rounded-xl border border-stone-200 space-y-1.5 max-h-32 overflow-y-auto">
                        {ucs
                          .filter(u => {
                            const escopo = newRegra.escopo || "transversal";
                            const anoVal = newRegra.anoCurricular || "todos";
                            const matchesAno = escopo === "transversal" || anoVal === "todos" || Number(u.anoCurricular) === Number(anoVal);
                            return matchesAno;
                          })
                          .map(u => {
                            const currentMotor = newRegra.config?.motor || {};
                            const restricoes = Array.isArray(currentMotor.restricoesUC) ? currentMotor.restricoesUC : [];
                            const isSelected = restricoes.some((r: any) => r.siglas?.includes(u.sigla));

                            const handleToggleUc = () => {
                              let currentRestricoes = [...restricoes];
                              if (isSelected) {
                                currentRestricoes = currentRestricoes.map((r: any) => {
                                  if (!r.siglas) return r;
                                  const nextSiglas = r.siglas.filter((s: string) => s !== u.sigla);
                            ...
                            return (
                              <label key={u.id} className="flex items-center gap-2 cursor-pointer p-0.5 hover:bg-stone-50 rounded">
                                <input
                                  type="checkbox"
                                  checked={isSelected}
                                  onChange={handleToggleUc}
                        ...
                        {ucs.filter(u => {
                          ...
                          const matchesAno = escopo === "transversal" || anoVal === "todos" || Number(u.anoCurricular) === Number(anoVal);
                          return matchesAno;
                        }).length === 0 && <span className="text-stone-400 italic text-[11px]">Nenhuma UC disponível.</span>}
                      </div>
                    </div>
                  </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="col-span-1 md:col-span-2 bg-amber-50/50 p-3 rounded-xl border border-amber-100 space-y-2 mt-2">
                        <span className="block text-[10px] font-bold text-amber-800 uppercase tracking-wider">Definições da Restrição para as UCs Selecionadas</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                              ...
                              {["Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado"].map(dia => {
                                ...
                                const handleToggleDia = () => {
                                  ...
                                  setNewRegra({
                                    ...
                                    config: {
                                      ...newRegra.config,
                                      motor: { ...currentMotor, restricoesUC: nextRestricoes }
                                    }
                                  });
                                };
                                return (
                                  <button
                                    ...
                                    type="button"
                                    onClick={handleToggleDia}
                                    className={`px-1.5 py-0.5 rounded text-[10px] font-semibold border ${isProibido ? "bg-amber-100 text-amber-800 border-amber-200" : "bg-white text-stone-500 border-stone-200 hover:bg-stone-50"}`}
                                  >
                                    {dia.slice(0, 3)}
                                  </button>
                                );
                          ...
                          <div>
                            ...
                            <div>
                              <label className="block text-[10px] font-bold text-stone-500 mb-1">Tipo de Aula</label>
                              <div className="flex flex-wrap gap-1">
                                {["T", "TP", "PL", "S"].map(tipo => {
                                  const isIncluded = restricoes.some((r: any) => r.tipos?.includes(tipo));
                                  const handleToggleTipo = () => {
